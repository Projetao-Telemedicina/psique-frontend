...
import { PacienteProfileForm } from '../components/PacienteProfileForm';
        ...
        <div className="flex-1 overflow-y-auto p-8 text-left">
          <div className="flex gap-8 items-start">
            <PacienteProfileForm
              ...
              isSaving={saving}
              onSave={handleSalvar}
              onCancel={() => { setIsEditing(false); setNewPassword(''); setCurrentPassword(''); }}
            ...
            <aside className={`w-[400px] flex flex-col gap-6 sticky top-0 transition-opacity ${isEditing ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
              <SideCard title="Compatibilidade" icon={<ClipboardList className="text-teal-500" />}>
                <p className="text-sm text-slate-500 mb-4">Responda o questionário para encontrar o melhor profissional para você.</p>
                <button 
                  onClick={() => navigate('/match')}
                  className="w-full py-3 bg-teal-600 text-white font-bold text-xs rounded-xl hover:bg-teal-700 transition-colors"
                >
              ...
              <SideCard title="Assinatura e Planos" icon={<Star className="text-emerald-500" />}>
                {subData?.subscription ? (
                  ...
                  <div className="text-center py-6">
                    <p className="text-sm text-slate-500 mb-4">Você não possui um plano ativo.</p>
                    <button
