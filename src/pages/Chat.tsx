        ...
        <div className="flex flex-1 gap-6 min-h-0">
          <div className="w-[350px] md:w-[421px] bg-[#EAEAEA] rounded-[15px] shadow-lg p-4 md:p-6 flex flex-col gap-6 min-h-0">
            ...
            {!isCreatingNew && (
              <div className="flex items-center gap-3 w-full h-[47px] px-4 py-3 bg-white/80 border border-slate-200 rounded-[24px]">
                ...
                <input 
                  ...
                  className="bg-transparent outline-none text-slate-600 text-[14px] w-full" 
                  value={searchQuery} 
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-1">
              {isCreatingNew ? (
                isLoadingApts ? <Loader2 className="animate-spin mx-auto text-[#5BB38A]" /> : uniqueAppointments.length > 0 ? (
                  uniqueAppointments.map((apt: any) => (
                    <div 
                      key={apt.id || apt.appointment_id || `${apt.professionalId || ''}-${apt.patientId || ''}`} 
                      onClick={() => createRoomMutation.mutate(apt)} 
                      className="p-4 bg-white rounded-xl cursor-pointer hover:bg-green-50 shadow-sm border border-slate-200 transition-all"
                    >
                      <p className="font-semibold text-slate-800">
                        {user?.role === 'PATIENT' ? (apt.professional?.user?.name || 'Profissional') : (apt.patient?.user?.name || 'Paciente')}
                      </p>
                      <p className="text-xs text-slate-400">Consulta agendada</p>
                    </div>
                  ))
                ) : (
                  <p className="text-center text-sm text-slate-500 py-8">Nenhuma consulta agendada.</p>
                )
              ) : isLoadingChats ? (
                ...
                filteredRooms.map((room) => (
                  ...
                  >
                    <div className="w-[75px] h-[73px] bg-gray-300 rounded-[15px] flex items-center justify-center font-bold overflow-hidden shrink-0 text-slate-600 text-xl">
                      {getContactAvatar(room) ? (
                        <img src={getContactAvatar(room) as string} alt={getRoomName(room)} className="w-full h-full object-cover" />
                      ) : (
                        (getRoomName(room) || '?')[0] || '?'
                      )}
                    </div>
                    <div className="flex-1 min-w-0 text-left">
                      <h2 className="font-semibold text-lg text-slate-800 truncate">{getRoomName(room)}</h2>
            ...
            {selectedChat ? (
              <>
                ...
                <div className="flex items-center gap-4 mb-8 text-left">
                   <div className="w-[75px] h-[73px] bg-gray-300 rounded-[15px] flex items-center justify-center font-bold overflow-hidden shrink-0 text-slate-600 text-xl">
                     {activeChatDetails && getContactAvatar(activeChatDetails) ? (
                       <img src={getContactAvatar(activeChatDetails) as string} alt="Avatar" className="w-full h-full object-cover" />
                     ) : (
                       (activeChatDetails ? getRoomName(activeChatDetails) : '?')?.[0] || '?'
                     )}
                   </div>
                   <h2 className="font-poppins font-semibold text-[24px] text-[#454545]">
                     {activeChatDetails ? getRoomName(activeChatDetails) : "Carregando..."}
                   </h2>
                </div>
                ...
                <form onSubmit={handleSendMessage} className="mt-6 flex items-center gap-3 bg-white/90 border border-slate-200 p-3 rounded-full">
                  ...
                  />
                  <Paperclip 
                    className="text-slate-400 cursor-pointer ml-2 hover:text-slate-600 transition-colors" 
