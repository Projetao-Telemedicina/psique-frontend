...
function CadastroProfissional() {
    ...
    return (
        <main className="login-page flex h-screen w-full overflow-hidden bg-snow">
            <HeroSection subtitle="Profissional" currentStep={cadastroConcluido ? 3 : etapa} totalSteps={3} />
                ...
                <div className="w-full max-w-[816px] flex flex-col justify-center py-10">
                    {cadastroConcluido ? (
                        <div className="text-center animate-in zoom-in duration-500 max-w-lg mx-auto">
                            ...
                            <h2 className="text-navy text-3xl font-bold mb-4">Cadastro enviado!</h2>
                            <p className="text-navy/70 text-lg mb-8">
                                Seus dados e documentos foram recebidos e estão sob análise. 
                                Nossa equipe verificará seu registro profissional em breve.
                            </p>
                            
                            <div className="bg-white p-6 rounded-2xl border border-navy/10 text-left mb-8 shadow-sm">
                                <h3 className="font-bold text-navy mb-3">Próximos passos:</h3>
                                <ul className="text-navy/70 space-y-2 list-disc pl-5">
                                    <li>Aguarde a validação do seu CRP/CRM.</li>
                                    <li>Notificaremos você por e-mail assim que aprovado.</li>
                                    <li>Após a aprovação, você poderá completar seu perfil e começar os atendimentos.</li>
                                </ul>
                            </div>

                            <button
                                onClick={() => { logout(); window.location.replace('/login'); }}
                                className="btn-primary rounded-full bg-navy w-full h-[56px] text-snow font-bold hover:bg-navy/90 transition-all"
                            >
                                    ...
                                    <div className="flex flex-col gap-[2.5vh] animate-in fade-in slide-in-from-right-4 duration-500">
                                        ...
                                        <div className="flex flex-col items-start w-full">
                                            ...
                                            <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer transition-all ${documentFile ? 'border-green-500 bg-green-50' : 'border-navy/20 hover:border-navy/40 bg-input-bg'}`}>
                                                ...
                                                <input type="file" className="hidden" accept=".pdf,image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) setDocumentFile(file); }} />
                                            </label>
                                        </div>
                                        <div className="flex flex-col md:flex-row gap-6">
                                            <div className="flex-1 flex flex-col items-start">
